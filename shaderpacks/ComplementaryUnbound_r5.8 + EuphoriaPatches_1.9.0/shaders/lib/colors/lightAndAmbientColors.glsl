    ...
    #include "/lib/shaderSettings/overworldBeams.glsl"
        ...
        vec3 endLightColor = clamp(mix(fogColor * 0.6 + 0.3 * normalize(fogColor + 0.0001) + 0.25 * (1.0 - endFogLumina), endLightTempC, inVanillaEnd * float(END_SKY_FOG_INFLUENCE)), 0.0, 1.5);
